<template>
  <div class="main" ref="main">
    ...
    <van-tabs
      ...
      :animated="true">
      <van-tab
        v-for="(cinemas, index) in dateList"
        :title="cinemas"
        :key="index">
        <div class="filter">
          <div class="tab" @click="show = !show">
            {{active==0?'全城':district}}<van-icon
              :name="show ? 'arrow-up' : 'arrow-down'"
              size="10px" />
          </div>
          <div class="tab">
            最近去过<van-icon name="arrow-down" size="10px" />
          </div>
        </div>
        <div
          :style="{ pointerEvents: show ? 'all' : 'none' }"
          class="popupContainer">
          <van-popup v-model="show" position="top" style="pointer-events: all">
            <ul>
              <li>
                <div
                  @click="selectDistrict('全城', 0)"
...
export default {
  ...
  mounted() {
    http({
      ...
      this.showCinemas = res.data.data.showCinemas.sort(
    ...
    if (this.$store.state.cinemasData.length === 0) {
      this.$store
        .dispatch("getCinemasData", this.$store.state.cityId)
        .then(this.getDistrictName)
    } else {
      // 有数据
  ...
  methods: {
    ...
    Calculation() {
      return this.showCinemas.map((data) =>
        getTime(data.showDate * 1000, ["week", "month", "day"])
      )
    },
    getDistrictName() {
      this.DistrictName = [
        ...new Set(this.$store.state.cinemasData.map((i) => i.districtName)),
      ]
    },
    ...
    filterShowCinemas(index){
      if(this.district == '全城'){
        ...
        return {cinemaList:this.$store.state.cinemasData.filter(i=>this.showCinemas[index].cinemaList.includes(i.cinemaId)&&i.districtName == this.district).map(i=>i.cinemaId)}
      }
    }
  },
}
